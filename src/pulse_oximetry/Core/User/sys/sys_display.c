...
uint32_t sys_display_init(sys_display_t *display, bsp_i2c_handle_t *i2c, uint8_t *dev_buffer)
{
  ...
  // Initialize UI
  drv_ssd1306_init(&(display->screen), i2c, SSD1306_I2C_ADDRESS, dev_buffer, SSD1306_WIDTH, SSD1306_HEIGHT);

  drv_ssd1306_write_string(&(display->screen), s_heart_rate, Font_6x8, DRV_SSD1306_COLOR_WHITE);

  drv_ssd1306_draw_bitmap(&(display->screen), MAX_WIDTH - BITMAP_WIDTH, 0, heart_icon, BITMAP_WIDTH,
                          BITMAP_HEIGHT, DRV_SSD1306_COLOR_WHITE);

  drv_ssd1306_draw_rectangle(&(display->screen), 0, MAX_HEIGHT - GRAPH_HEIGHT - BITMAP_HEIGHT, MAX_WIDTH,
                             MAX_HEIGHT - 9, DRV_SSD1306_COLOR_WHITE);
...
uint32_t sys_display_update_heart_rate(sys_display_t *display, uint8_t heart_rate)
{
  ...
  drv_ssd1306_set_cursor(&(display->screen), Font_6x8.width * 11, 0);
  drv_ssd1306_write_string(&(display->screen), heart_rate_update, Font_6x8, DRV_SSD1306_COLOR_WHITE);
...
uint32_t sys_display_update_ppg_signal(sys_display_t *display, cbuffer_t *signal_buf)
{
  ...
  for (uint8_t i = 0; i < ((MAX_WIDTH + 1) / RATIO); i++)
  {
    ...
    drv_ssd1306_draw_line(&display->screen, s_graph_pos_x, s_graph_pre_pos_y, (s_graph_pos_x + RATIO),
                          s_graph_pos_y, DRV_SSD1306_COLOR_WHITE);
    s_graph_pos_x += RATIO;
...
uint32_t sys_display_update_threshold(sys_display_t *display, uint8_t *threshold)
{
  ...
  char high[4];
  uint8_t low_bytes =
    sprintf(low, "%03d", (*(threshold) < *(threshold + 1) ? *(threshold) : *(threshold + 1)));
  uint8_t high_bytes =
    sprintf(high, "%03d", (*(threshold) > *(threshold + 1) ? *(threshold) : *(threshold + 1)));

  memcpy(s_low_threshold + 2, low, low_bytes);
  memcpy(s_high_threshold + 2, high, high_bytes);

  drv_ssd1306_set_cursor(&(display->screen), MAX_WIDTH - 6 * 5 * 2 - 2, MAX_HEIGHT - BITMAP_HEIGHT + 1);

  drv_ssd1306_write_string(&(display->screen), s_high_threshold, Font_6x8, DRV_SSD1306_COLOR_WHITE);

  drv_ssd1306_set_cursor(&(display->screen), MAX_WIDTH - 6 * 5, MAX_HEIGHT - BITMAP_HEIGHT + 1);
...
uint32_t sys_display_show_noti(sys_display_t *display, char *noti)
{
  ...
  drv_ssd1306_write_string(&(display->screen),
                           ...
                           Font_6x8, DRV_SSD1306_COLOR_WHITE);
