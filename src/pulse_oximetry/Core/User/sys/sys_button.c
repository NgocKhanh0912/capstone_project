...
uint32_t sys_button_manage()
{
  ...
  case SYS_BUTTON_FSM_STATE_PRESS:
  {
    ...
    uint32_t delta_t      = current_tick - previous_end_time;
...
static void sys_button_detect_edge(uint16_t exti_line)
{
  ...
  {
    if (s_button.transient_state == SYS_BUTTON_STATE_STABLE)
    {
...
static void sys_button_debound()
{
  ...
  {
    s_button.dbutton.current_state = bsp_gpio_read_pin(s_button.dbutton.port, s_button.dbutton.pin);
    s_button.transient_state       = SYS_BUTTON_STATE_STABLE;
  }
