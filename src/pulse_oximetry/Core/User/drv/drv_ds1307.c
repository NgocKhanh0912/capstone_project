...
drv_ds1307_status_t drv_ds1307_set_time(drv_ds1307_t *ds1307)
{
  ...
  __ASSERT((ds1307->second >= MINIMUM_SECOND) && (ds1307->second <= MAXIMUM_SECOND), DRV_DS1307_ERROR);

  __ASSERT((ds1307->minute >= MINIMUM_MINUTE) && (ds1307->minute <= MAXIMUM_MINUTE), DRV_DS1307_ERROR);
  ...
  *(tx_time_buffer + 2) = convert_decimal_to_bcd(ds1307->hour);
  ...
  {
    __ASSERT((ds1307->hour >= MINIMUM_HOUR_MODE_12H) && (ds1307->hour <= MAXIMUM_HOUR_MODE_12H),
             DRV_DS1307_ERROR);
  ...
  {
    __ASSERT((ds1307->hour >= MINIMUM_HOUR_MODE_24H) && (ds1307->hour <= MAXIMUM_HOUR_MODE_24H),
             DRV_DS1307_ERROR);
...
drv_ds1307_status_t drv_ds1307_get_time(drv_ds1307_t *ds1307)
{
  ...
  __ASSERT((ds1307->second >= MINIMUM_MINUTE) && (ds1307->second <= MAXIMUM_MINUTE), DRV_DS1307_ERROR);
...
drv_ds1307_status_t drv_ds1307_set_date(drv_ds1307_t *ds1307)
{
  ...
  __ASSERT((ds1307->day >= MINIMUM_DAY) && (ds1307->day <= MAXIMUM_DAY), DRV_DS1307_ERROR);

  __ASSERT((ds1307->date >= MINIMUM_DATE) && (ds1307->date <= MAXIMUM_DATE), DRV_DS1307_ERROR);

  __ASSERT((ds1307->month >= MINIMUM_MONTH) && (ds1307->month <= MAXIMUM_MONTH), DRV_DS1307_ERROR);

  __ASSERT((ds1307->year >= MINIMUM_YEAR) && (ds1307->year <= MAXIMUM_YEAR), DRV_DS1307_ERROR);
